...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const injectStyles = () => {
    if(!document.getElementById('rgp-injected-styles')) {
      ...
      style.textContent = `
        ...
        /* --- PRINT LAYOUT & PAGE BREAK FIXES --- */
        @media print {
          body * { visibility: hidden; }
          #receiptPaper, #receiptPaper * { visibility: visible; }
          
          /* Start exactly from top */
          html, body {
            margin: 0 !important;
            padding: 0 !important;
          ...
          /* Prevent specific sections from breaking unnecessarily across pages */
          #receiptPaper tr,
          #receiptPaper th,
          #receiptPaper td,
          .card-terms,
  ...
  const injectUIElements = () => {
    ...
    if (btnReset && !document.getElementById('btnPrintView')) {
      btnReset.insertAdjacentHTML('beforebegin', `
        <button id="btnPrintView" type="button" class="btn-secondary" style="width:100%; margin-bottom:10px; padding:10px; background:#475569; color:white; border:none; border-radius:4px; font-weight:bold; cursor:pointer;">🖨️ Print / Download PDF</button>
      `);
    }
    ...
    // ==========================================
    // PDF SETUP DIALOG HTML INJECTION
    // ==========================================
    if(!document.getElementById('pdfSetupModal')) {
      const setupModalHTML = `
        <div id="pdfSetupModal" class="modal-overlay" style="display:none; position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:10000; justify-content:center; align-items:center;">
          <div class="modal-content" style="background:#fff; color:#333; padding:25px; border-radius:8px; width:480px; max-width:95%; box-shadow: 0 10px 25px rgba(0,0,0,0.2);">
            <h3 style="margin-top:0; border-bottom:1px solid #eee; padding-bottom:10px;">📄 PDF Page Setup</h3>
            
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-top: 15px;">
              <label style="font-weight:bold; font-size:13px;">Paper Size:
                <select id="pdfPaperSize" style="width:100%; padding:8px; margin-top:4px; border:1px solid #ccc; border-radius:4px;">
              ...
              <label style="font-weight:bold; font-size:13px;">Orientation:
                <select id="pdfOrientation" style="width:100%; padding:8px; margin-top:4px; border:1px solid #ccc; border-radius:4px;">
                  <option value="portrait">Portrait</option>
                  <option value="landscape">Landscape</option>
                </select>
              </label>

              <label style="font-weight:bold; font-size:13px;">Margins:
                <select id="pdfMargins" style="width:100%; padding:8px; margin-top:4px; border:1px solid #ccc; border-radius:4px;">
                  <option value="10mm">Normal (10mm)</option>
                  <option value="5mm">Narrow (5mm)</option>
                  <option value="20mm">Wide (20mm)</option>
                  <option value="0mm">None (0mm)</option>
                </select>
              </label>

              <label style="font-weight:bold; font-size:13px;">Scale:
                <select id="pdfScale" style="width:100%; padding:8px; margin-top:4px; border:1px solid #ccc; border-radius:4px;">
                  <option value="1">100%</option>
                  <option value="0.95">Fit to Page (95%)</option>
                  <option value="0.8">Custom (80%)</option>
                </select>
              </label>
            </div>

            <div style="margin-top: 20px; display: grid; gap: 10px; font-size:14px; background:#f8fafc; padding:15px; border-radius:6px; border:1px solid #e2e8f0;">
              <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="pdfPageNumbers"> Enable Page Numbers</label>
              <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="pdfWatermark" checked> Enable Watermark</label>
              <label style="display:flex; align-items:center; gap:8px;"><input type="checkbox" id="pdfBgColors" checked> Show Background Colors & Graphics</label>
            </div>

            <div style="margin-top:15px; padding:12px; background:#e0f2fe; border:1px dashed #7dd3fc; text-align:center; font-size:14px; color:#0369a1; border-radius:4px;">
              Estimated Generated Pages: <strong id="pdfEstPages" style="font-size:16px;">1</strong>
            </div>

            <div style="display:flex; justify-content:space-between; align-items:center; margin-top:25px;">
              <button id="btnCancelPdfModal" type="button" class="btn-secondary" style="padding:10px 16px; border:1px solid #ccc; background:#f1f5f9; border-radius:4px; cursor:pointer; font-weight:bold;">Cancel</button>
              <button id="btnConfirmGeneratePdf" type="button" style="padding:10px 20px; background:#2563eb; color:white; border:none; border-radius:4px; font-weight:bold; cursor:pointer; font-size:15px;">🖨️ Generate PDF</button>
            </div>
          </div>
        </div>
      `;
      ...
      document.getElementById('btnCancelPdfModal').addEventListener('click', () => {
        document.getElementById('pdfSetupModal').style.display = 'none';
      });

      document.getElementById('btnConfirmGeneratePdf').addEventListener('click', () => {
        const settings = {
          size: document.getElementById('pdfPaperSize').value,
          orientation: document.getElementById('pdfOrientation').value,
          margins: document.getElementById('pdfMargins').value,
          scale: document.getElementById('pdfScale').value,
          numbers: document.getElementById('pdfPageNumbers').checked,
          watermark: document.getElementById('pdfWatermark').checked,
          bg: document.getElementById('pdfBgColors').checked
        };
        localStorage.setItem('rgp_pdf_settings', JSON.stringify(settings));

        let pageCss = `@page { size: ${settings.size} ${settings.orientation}; margin: ${settings.margins}; }`;
        const dynamicStyleId = 'rgp-dynamic-pdf-style';
        ...
        let extraCss = '';
        if(settings.bg) {
          extraCss += `@media print { * { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; } }`;
        }
        if(!settings.watermark && cache.prevWatermark) {
          extraCss += `@media print { #prevWatermark { display: none !important; } }`;
        }
        if(settings.numbers) {
           extraCss += `
             @media print {
        ...
        styleTag.textContent = pageCss + extraCss;
        document.documentElement.style.setProperty('--pdf-scale', settings.scale);
        document.getElementById('pdfSetupModal').style.display = 'none';
    ...
    if (printBtnNode) {
        printBtnNode.addEventListener('click', () => {
            // Bypass rigid validation - only highlight empty fields visually
            validateForm(); 
            updatePreview();
            openPdfSetupModal();
        });
  ...
  document.getElementById('tabPreview')?.addEventListener('click', (e) => {
    // PREVIEW FIX: Never block Preview because of empty fields.
    validateForm(); // Call to visually highlight missing inputs, but DO NOT block switching
    e.target.classList.add('active');
  ...
  document.getElementById('btnSaveHistory')?.addEventListener('click', () => {
    // History saving still validates correctly before committing bad data
