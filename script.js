...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const injectUIElements = () => {
    ...
    if (btnDownload && !document.getElementById('pdfQuality')) {
      btnDownload.insertAdjacentHTML('beforebegin', `
        <select id="pdfQuality" style="width:100%; margin-bottom:10px; padding:8px; border-radius:4px; border:1px solid #ccc;">
          <option value="high">High Quality PDF (Recommended)</option>
          <option value="standard">Standard Quality (Smaller Size)</option>
          <option value="print">Print Quality (Best Detail)</option>
        </select>
      `);
      cache.pdfQuality = document.getElementById('pdfQuality');
    }
    
    // Injecting the Print Invoice Button dynamically
    if (btnDownload && !document.getElementById('btnPrintView')) {
      btnDownload.insertAdjacentHTML('afterend', `
        <button id="btnPrintView" type="button" class="btn-secondary" style="width:100%; margin-top:10px; padding:10px; background:#475569; color:white; border:none; border-radius:4px; font-weight:bold; cursor:pointer;">🖨️ Print Invoice</button>
      `);
    ...
    if (receiptPaper && !document.getElementById('prevWatermark')) {
      ...
      // Inject Status Container BEFORE the receiptPaper
  ...
  const calculateCalculatedDueDate = () => {
    ...
    if (!isNaN(baseDate.getTime())) {
      ...
      cache.dueDate.value = baseDate.toISOString().split('T')[0];
      updatePreview();
    }
  };
  ...
  document.getElementById('libraryTargetSelect')?.addEventListener('change', (e) => {
    ...
    if (activeItem) {
      if (activeItem.notes) cache.notes.value = activeItem.notes;
      if (activeItem.terms) cache.terms.value = activeItem.terms;
      if (activeItem.bank) cache.bankDetails.value = activeItem.bank;
      updatePreview();
    }
  });
  ...
  const applySnapshotToForm = (snap) => {
    if (!snap) return;
    if (snap.bizName !== undefined) cache.bizName.value = snap.bizName;
    if (snap.bizEmail !== undefined) cache.bizEmail.value = snap.bizEmail;
    if (snap.bizPhone !== undefined) cache.bizPhone.value = snap.bizPhone;
    if (snap.bizAddress !== undefined) cache.bizAddress.value = snap.bizAddress;
    if (snap.custName !== undefined) cache.custName.value = snap.custName;
    if (snap.custCompany !== undefined) cache.custCompany.value = snap.custCompany;
    if (snap.custEmail !== undefined) cache.custEmail.value = snap.custEmail;
    if (snap.custPhone !== undefined) cache.custPhone.value = snap.custPhone;
    if (snap.custAddress !== undefined) cache.custAddress.value = snap.custAddress;
    if (snap.receiptNumber !== undefined) cache.receiptNumber.value = snap.receiptNumber;
    if (snap.issueDate !== undefined) cache.issueDate.value = snap.issueDate;
    ...
    if (snap.currency !== undefined) cache.currencySelect.value = snap.currency;
    if (snap.discount !== undefined) cache.discountVal.value = snap.discount;
    if (snap.tax !== undefined) cache.taxRate.value = snap.tax;
    if (snap.taxLabel !== undefined) cache.taxLabelInput.value = snap.taxLabel;
    if (snap.shipping !== undefined) cache.shippingCost.value = snap.shipping;
    if (snap.themeColor !== undefined) cache.themeColorSelect.value = snap.themeColor;
    if (snap.paymentArch !== undefined) cache.paymentArchType.value = snap.paymentArch;
    if (snap.bankDetails !== undefined) cache.bankDetails.value = snap.bankDetails;
    if (snap.payUrl !== undefined) cache.payUrl.value = snap.payUrl;
    if (snap.payMethodText !== undefined) cache.payMethod.value = snap.payMethodText;
    if (snap.notes !== undefined) cache.notes.value = snap.notes;
    if (snap.terms !== undefined) cache.terms.value = snap.terms;
  ...
  const runSmartFieldValidation = (field, validationType) => {
    ...
    if (value === "") {
      ...
      if (validationType === 'email') {
        ...
        if (cache.issueDate.value && cache.dueDate && cache.dueDate.value) {
  ...
  const validateForm = () => {
    let formsValid = true;
    if (!runSmartFieldValidation(cache.bizName, 'string')) formsValid = false;
    if (!runSmartFieldValidation(cache.custName, 'string')) formsValid = false;
  ...
  if(savedProfile.bizPhone) cache.bizPhone.value = savedProfile.bizPhone;
  if(savedProfile.bizAddress) cache.bizAddress.value = savedProfile.bizAddress;
  ...
  if (btnDup) {
    btnDup.addEventListener('click', () => {
      ...
      cache.receiptNumber.value = generateAutoNumber();
      cache.issueDate.valueAsDate = new Date();
  ...
  cache.paymentArchType.addEventListener('change', (e) => {
    if(e.target.value === 'bank') {
      cache.bankFields.style.display = 'block';
      cache.stripeFields.style.display = 'none';
    } else {
      cache.bankFields.style.display = 'none';
      cache.stripeFields.style.display = 'block';
    }
    updatePreview();
  });
  ...
  const updateBankString = () => {
    ...
    cache.bankDetails.value = str.trim();
    updatePreview();
  };
  ...
  const renderItemsEditor = () => {
    ...
    state.items.forEach((item, index) => {
      ...
      tr.innerHTML = `
        ...
        <td><input type="text" class="item-desc req-field" data-id="${item.id}" value="${sanitizeHTML(item.desc)}" list="itemMemoryList" placeholder="Item Description"></td>
  ...
  const updatePreview = () => {
    ...
    cache.prevTaxLabel.textContent = cache.taxLabelInput.value || 'Tax';
    ...
    cache.prevPayMethod.textContent = payMethod;
    if(cache.paymentArchType.value === 'bank') {
      cache.prevBankDetails.innerHTML = bankLines.join('<br>');
    ...
    cache.prevSubtotal.textContent = formatMoney(subtotal);
    cache.rowDiscount.style.display = d > 0 ? 'flex' : 'none';
    if(d>0) cache.prevDiscount.textContent = `-${formatMoney(d)}`;
    cache.rowTax.style.display = taxAmt > 0 ? 'flex' : 'none';
    if(taxAmt>0) cache.prevTax.textContent = formatMoney(taxAmt);
    cache.rowShipping.style.display = s > 0 ? 'flex' : 'none';
    if(s>0) cache.prevShipping.textContent = formatMoney(s);
    cache.prevTotal.textContent = formatMoney(gTotal);
    ...
    setTimeout(() => {
      ...
      if (paper && statusEl) {
        ...
        if (usagePercent <= 100) {
            ...
            statusEl.innerHTML = `⚠ Invoice exceeds one A4 page. The generated PDF will contain 2 pages. <br><span style="font-weight:normal; font-size:13px;">${usagePercent}% → Second page required</span>`;
  ...
  // ==========================================
  // SCALED ANTI-CLIPPING HIGHEST QUALITY PDF ENGINE
  // ==========================================
  document.getElementById('btnDownloadPDF').addEventListener('click', () => {
    if (validateForm()) {
      updatePreview();

      // Ensure Pre-check block for overflow warning
      const statusEl = document.getElementById('a4-overflow-status');
      if (statusEl && statusEl.getAttribute('data-exceeds') === 'true') {
        const proceed = confirm("⚠ Your invoice exceeds one A4 page. The generated PDF will contain 2 pages.\n\nDo you want to continue generating the PDF?");
        if (!proceed) return;
      }
      ...
      // Updated configuration for anti-clipping & robust layout extraction
      const opt = {
        margin: [10, 10, 10, 10], 
        filename: fileName,
      ...
      btn.innerHTML = '⏳ Rendering Vector Layers...';
      
      html2pdf().set(opt).from(element).save().then(() => {
        btn.innerHTML = originalText;
      }).catch(err => {
        console.error("PDF engine compiler interface failure: ", err);
        btn.innerHTML = originalText;
      });
    }
  });
  ...
  document.getElementById('savedClientsDropdown').addEventListener('change', (e) => {
    if(e.target.value !== "") {
      ...
      cache.custName.value = c.custName || '';
      cache.custCompany.value = c.custCompany || '';
      cache.custEmail.value = c.custEmail || '';
      cache.custPhone.value = c.custPhone || '';
      cache.custAddress.value = c.custAddress || '';
      updatePreview();
    }
  });
  ...
  document.getElementById('savedPaymentsDropdown').addEventListener('change', (e) => {
    if(e.target.value !== "") {
      ...
      cache.paymentArchType.value = p.payArch;
      cache.bankDetails.value = p.bank || '';
      cache.payUrl.value = p.stripe || '';
      cache.payMethod.value = p.payMethodText || '';
      ...
      cache.paymentArchType.dispatchEvent(new Event('change'));
      updatePreview();
    }
  });
  ...
  window.app = {
    loadHistoryItem: (i) => {
      ...
      if(h.currency) cache.currencySelect.value = h.currency;
      if(h.notes !== undefined) cache.notes.value = h.notes;
      if(h.terms !== undefined) cache.terms.value = h.terms;
      ...
      cache.receiptType.value = h.type || 'Invoice';
      cache.discountVal.value = h.discount || '';
      cache.taxRate.value = h.tax || '';
      cache.shippingCost.value = h.shipping || '';
      ...
      if(h.payArch) cache.paymentArchType.value = h.payArch;
      if(h.bank) {
      ...
      if(h.stripe) cache.payUrl.value = h.stripe;
      if(h.payMethodText) cache.payMethod.value = h.payMethodText;
    ...
    showAuth: (type) => {
      ...
      document.getElementById('authSwitchText').innerHTML = type === 'login' ? `Don't have an account? <a onclick="app.showAuth('signup')">Sign up</a>` : `Already have an account? <a onclick="app.showAuth('login')">Login</a>`;
