...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updatePreview = () => {
    ...
    state.items.forEach((item) => {
      ...
      let t = p * q;
      subtotal += t;
      cache.prevItemsBody.innerHTML += `<tr><td style="text-align:center; color:#64748b; font-weight:600;">${indexCounter}</td><td style="font-weight:500;">${sanitizeHTML(item.desc)}</td><td style="text-align:center;">${q||''}</td><td style="text-align:right;">${formatMoney(p)}</td><td style="text-align:right; font-weight:700; color:#0f172a;">${formatMoney(t)}</td></tr>`;
      indexCounter++;
    });
